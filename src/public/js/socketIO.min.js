   ...
   socket.on('server:resp', data => {      
      if (data.msg) {
         Swal.fire({
            ...
            if (result.value) {
               const boletoNum = ($.trim(numBoleto.value) < 10) 
                  ? `0${$.trim(numBoleto.value)}` 
                  : $.trim(numBoleto.value);
            
               document.querySelector(`.C${$.trim(numBoleto.value)}`).classList.add('vendido');
               document.querySelector(`.C${$.trim(numBoleto.value)}`).innerHTML = `
                  <div class="number">
                     ${boletoNum}
                  </div>
      ...
      if (data.msg === 'not') {
         Swal.fire({
            ...
            html: '<p style="font-size: 1rem;">El boleto ingresado no es válido, el rango a ingresar debe de ser entre el 0 al 99...</p>',
            icon: 'error',
            allowOutsideClick: false
         });
      }
...
socket.on('server:gen', data => {
   ...
   if (data) {
      Eggy({
         title:  `Boleto ${data.numBoleto} vendido`,
         message:  `${data.vendedor} vendió el boleto ${data.numBoleto}`,
         position: 'top-right',
         type: 'info'
      });
